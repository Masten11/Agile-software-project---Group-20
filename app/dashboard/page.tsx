      ...
      >
        ...
        {!logged && (
          <div
            ...
            style={{
              border: "1px solid var(--accent-green-badge-border)",
            }}
          >
            <Plus
              size={16}
...
function EcoScoreChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error("Failed to fetch eco score data");
        }
        ...
        console.error("Failed to load Eco Score chart data:", err);
      } finally {
  ...
  return (
    <div
      className="rounded-2xl p-6 relative shadow-sm"
      style={{
        background: "var(--bg-card)",
    ...
    >
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative">
          ...
          {dropdownOpen && (
            <div
              className="absolute right-0 mt-2 rounded-xl overflow-hidden z-50 shadow-xl"
              style={{
...
export default function DashboardPage() {
  ...
  try {
    ...
    // ── Eco score history for chart + stats ──
    const historyRes = await fetch("/api/historical-data");

    if (historyRes.ok) {
      const historyData = await historyRes.json();

      const eco = historyData.eco_score;
    ...
    // ── Logged categories today ──
    if (data.activities && data.activities.length > 0) {
      const uniqueCategories = Array.from(
        new Set(
          data.activities.map((a: { category: string }) => {
            const c = a.category.toLowerCase();

            if (c === "shower" || c === "dishwasher" || c === "washing_machine" || c === "washingmachine") {
  return "household";
}if (
              c === "shower" ||
              c === "dishwasher" ||
    ...
    if (user) {
      ...
      if (history && history.length > 0) {
        ...
        if (
          ...
          while (
            uniqueDates.includes(getOffsetDate(offset))
          ) {
            calculatedStreak++;
            offset++;
          }
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <div
  className="flex gap-4 sm:gap-6 mt-6 pt-6 w-full max-w-sm"
  ...
  {[
    {
      ...
      val:
        weeklyData.length >= 2
          ? weeklyData[weeklyData.length - 2]?.value
          : "—",
    },
